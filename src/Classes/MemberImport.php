...
class MemberImport extends Backend
{
    protected string $strTemplate = 'be_member_import';

    public function run(): string
    {
        $tpl = new BackendTemplate($this->strTemplate);
        $tpl->message = Message::generate();
        $tpl->report = null;
        $tpl->emailSubject = Config::get('bcs_member_import_email_subject') ?: 'Reset your account password';
        $tpl->emailBody = Config::get('bcs_member_import_email_body') ?: "Hi {{firstname}},\n\nPlease set your password using this link:\n\n{{reset_link}}\n\nThis link will expire soon.";
        ...
        if (Input::post('bcs_import') !== null) {
            ...
            $tpl->report = $this->handleUploadAndImport();
        }
    ...
    public function __invoke(): string{return $this->run();}
}
